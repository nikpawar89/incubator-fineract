...
public class CBLPCommandFromApiJsonDeserializer {
	...
	public void validateForCreate(final String json, final Long cb_id) {
		...
		final boolean skip_creditcheck_in_failure = this.fromApiJsonHelper
				.extractBooleanNamed("skip_creditcheck_in_failure", element);
		baseDataValidator.reset().parameter("skip_creditcheck_in_failure").value(skip_creditcheck_in_failure).notBlank()
				.trueOrFalseRequired(skip_creditcheck_in_failure);
