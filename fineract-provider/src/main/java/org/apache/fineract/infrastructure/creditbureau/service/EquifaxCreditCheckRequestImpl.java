...
import org.apache.fineract.infrastructure.creditbureau.data.AccountTypeData;
import org.apache.fineract.infrastructure.creditbureau.data.EquifaxReportData;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.servicedefs._1.CreditReportWSInquiryPortType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.AccountDetailsType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.AccountType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.AdditionalNameTypeDetails;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.CreditReportSummaryType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.FamilyInfo;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.IDAndContactType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.IDType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.IdentificationType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryCommonInputPhoneType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryPhoneType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryRequestType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryResponseType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.MaritalStatusOptions;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.ReportFormatOptions;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.ReportType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.RequestBodyType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.RequestHeaderType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.ScoreType;
import org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.StateCodeOptions;
...
public class EquifaxCreditCheckRequestImpl implements EquifaxCreditCheckRequest {
	...
	private static CreditBureauReadConfigurationService cbconfig;

	private static CreditBureauLpMappingReadPlatformService cbMapping;

	private static LoanReadPlatformService loanService;
	...
	@Autowired
	public EquifaxCreditCheckRequestImpl(CreditBureauReadConfigurationService cbconfig,
			CreditBureauLpMappingReadPlatformService cbMapping, LoanReadPlatformService loanService) {
		this.cbconfig = cbconfig;
		this.cbMapping = cbMapping;
		this.loanService = loanService;
	}

	private static void populateUATRetailRequestHeaderType(RequestHeaderType requestHeader, long loanAccountNumber) {
		...
		Map<String, String> cbConfiguration = cbconfig.retrieveConfigMap(cbOrgId);

		// plugging in creditbureau configuration

		// TODO: Update your value :: Company id shared by Equifax
		requestHeader.setCustomerId(Integer.parseInt(cbConfiguration.get("customerId")));
		requestHeader.setUserId(cbConfiguration.get("userId"));
		// requestHeader.setCustRefField("1234");
		// TODO: Update your value :: Member Number shared by Equifax
		requestHeader.setMemberNumber(cbConfiguration.get("memberNumber"));
		// TODO: Update your value :: password shared by Equifax
		requestHeader.setPassword(cbConfiguration.get("password"));
		// TODO: Update your value :: desired product code - This is dynamic
		// field based on business requirement
		// requestHeader.setProductCode("PCS");
		// TODO: Update your value :: desired product code - This is dynamic
		// field based on business requirement
		requestHeader.setProductCode(cbConfiguration.get("productCode"));
		requestHeader.setProductVersion(cbConfiguration.get("productVersion"));
		requestHeader.setReportFormat(ReportFormatOptions.XML);
		// TODO: Update your value :: Security digit shared by equifax
		requestHeader.setSecurityCode(cbConfiguration.get("securityCode"));
		// TODO: Update your value :: user id shared by Equifax

	}


	// plugging in equifax with cbconfig
	@Override
	public EquifaxReportData processRequest(long loanAccountNumber) {
		...
		// QName qname = new QName("http://stub.ists.mn.com/",
		// "StubIstsImplService");
		...
		// ****************** create Header and set it to request****************************
		RequestHeaderType requestHeader = new RequestHeaderType();
		populateUATRetailRequestHeaderType(requestHeader, loanAccountNumber);
		input.setRequestHeader(requestHeader);
		
		// ****************** create body and set it to request****************************
	...
	private RequestBodyType constructRetailRequest() {
		...
		/*
		 * requestBody.setMiddleName("M"); requestBody.setLastName("T");
		 */
		...
		/*
		 * GregorianCalendar c = new GregorianCalendar(); c.set(1962, 01, 05);
		 * try {
		 * 
		 ...
		 * 
		 * } catch(DatatypeConfigurationException e) { System.out.println(e); }
		 * requestBody.setRationCard("03/W/7820489");
		 */

		/*
		 * LandlineType homePhone = new LandlineType(); // TODO: Update your
		 * value homePhone.setPhoneNumber("2987987"); // TODO: Update your value
	...
	private static EquifaxReportData printResponse(InquiryResponseType responseType) throws JAXBException {

		JAXBContext jc = JAXBContext.newInstance(
				org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryResponseType.class);

		IDAndContactType idAndContactType = responseType.getReportData().getIDAndContactInfo();
		if (idAndContactType != null) {
			IdentificationType identificationType = idAndContactType.getIdentityInfo();
			if (identificationType != null) {
				List<IDType> panIds = identificationType.getPANId();

				for (IDType idType : panIds) {
					System.out.println("############################### PAN ID ###############################");
		...
		/*
		 * Response
		 * creditReport=(Response)unmarshaller.unmarshal(responseReader);
		 * 
		 * 
		 * return response;
		 */
		...
		for (AccountType account : accounts) {
			...
			accountData.add(AccountTypeData.instance(account.getInstitution(), account.getAccountNumber(),
					account.getDisbursedAmount(), account.getLastPayment(), account.getInstallmentAmount(),
					account.getAccountType(), account.getCurrentBalance()));
		}
	...
	private static void printRequest(InquiryRequestType requestType) throws JAXBException {
		...
		JAXBContext jc = JAXBContext.newInstance(
				org.apache.fineract.infrastructure.creditbureau.equifax.services.eport.ws.schemas._1.InquiryRequestType.class);
